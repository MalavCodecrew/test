...
class VariantSelects extends HTMLElement {
  ...
  updateVariantInput() {
    ...
    const quickInfoForm = document.querySelectorAll(
      `.form`
    );
    ...
    quickInfoForm.forEach((productForm) => {
      const input = productForm.querySelector('input[name="id"]');
      input.value = this.currentVariant.id;
      input.dispatchEvent(new Event('change', { bubbles: true }));
    });
...
class QuickInfoVariantSelects extends HTMLElement {
  constructor() {
    super();
    this.addEventListener('change', this.onVariantChange);
  }
  ...
  updateURL() {
    console.log("thisssss",this.dataset)
    if (!this.currentVariant || this.dataset.updateUrl === 'false') return;
    window.history.replaceState({}, '', `${this.dataset.url}?variant=${this.currentVariant.id}`);
  }
...
customElements.define('quick-info-variant-selects', QuickInfoVariantSelects);
